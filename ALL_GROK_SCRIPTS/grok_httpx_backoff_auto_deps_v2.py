...
# Version: Updated by Cortana for timeout fixes, auto-setup, auto-run, and dynamic paths
...
class GrokClient:
    def __init__(self):
        ...
        # Load credentials from environment variables (loaded from .env file in SCRIPT_DIR)
        # or fallback to placeholders if not found (though .env is expected).
        self.cookies = {
            ...
            "sso-rw": os.getenv("GROK_SSO_RW", os.getenv("GROK_SSO", "YOUR_SSO_RW_FALLBACK")), # Fallback sso-rw to sso if not defined
        ...
        self.headers = {
            ...
            "sec-ch-ua": '"Brave";v="135", "Not-A.Brand";v="8", "Chromium";v="135"', # Example, adjust if needed
            "sec-ch-ua-mobile": "?0",
            "sec-ch-ua-platform": '"macOS"', # Example, adjust if needed
    ...
    def _stream_request(self, url: str, payload: dict) -> Generator[Dict, None, None]:
        logger.info(f"Streaming request to {url} with payload: {{message: {payload.get('message', '')[:50]}...}}")
                        ...
                        if "result" in json_obj:
                            ...
                            if "response" in result_data:
                                ...
                                if "modelResponse" in response_data and "message" in response_data["modelResponse"]:
                                    ...
                                    return # Stop after final message
    ...
    def continue_conversation(self, conversation_id: str, message_content: str) -> Generator[Dict, None, None]:
        ...
        # First, yield the conversation_id so the caller knows which one it is
        yield {"type": "conversation_id", "id": conversation_id}
        yield from self._stream_request(url, payload)

grok_client = GrokClient()
executor = ThreadPoolExecutor(max_workers=10) # For running blocking generators in threads

async def run_blocking_generator_in_thread(gen_func, *args) -> AsyncGenerator[Dict, None]:
    loop = asyncio.get_event_loop()
    gen = await loop.run_in_executor(executor, gen_func, *args)
    for item in gen:
        yield item
        await asyncio.sleep(0) # Allow other tasks to run

async def create_new_conversation_async(messages: List[Dict[str, Any]]) -> AsyncGenerator[Dict, None]:
    system_prompt_parts = [msg["content"] for msg in messages if msg.get("role") == "system" and "content" in msg]
    user_message_parts = [msg["content"] for msg in messages if msg.get("role") == "user" and "content" in msg]
    
    if not user_message_parts: raise ValueError("No user message found")
    user_message_content = "\n".join(user_message_parts) # Combine if multiple user messages
    
    formatted_message = user_message_content
    if system_prompt_parts:
        system_context = "\n".join(system_prompt_parts)
        system_preview = system_context[:2000] + ("..." if len(system_context) > 2000 else "")
        formatted_message = f"SYSTEM CONTEXT:\n{system_preview}\n\nUSER MESSAGE:\n{user_message_content}"
        
    async for item in run_blocking_generator_in_thread(grok_client.new_conversation, formatted_message):
        yield item
...
async def format_openai_stream_response(response_gen: AsyncGenerator[Dict, None], request_model: Optional[str]) -> AsyncGenerator[str, None]:
    ...
    async for item in response_gen:
        ...
        elif item["type"] == "final_message": # This indicates the end of useful tokens for streaming
            break # The final chunk below will handle finish_reason
        # conversation_id is handled by the caller now for mapping
...
async def collect_full_openai_response(response_gen: AsyncGenerator[Dict, None], request_model: Optional[str], request_messages: List[Dict[str,Any]]):
    ...
    grok_conversation_id_for_response = None # Capture the ID if it comes through

    async for item in response_gen:
        if item["type"] == "conversation_id":
            grok_conversation_id_for_response = item["id"]
        elif item["type"] == "token":
            full_response_content = item["accumulated_message"] # Use the full accumulated message
        elif item["type"] == "final_message":
            full_response_content = item["message"] # Prefer final_message if available
            break # Stop once final message is received
            
    if not full_response_content: # If only tokens were received, use the last accumulated.
        # This path might not be hit often if final_message is always sent.
        logger.warning("No 'final_message' received, using accumulated tokens for non-streaming.")

    prompt_tokens = sum(len(m.get("content", "")) for m in request_messages) // 4 # Rough estimate
    completion_tokens = len(full_response_content) // 4 # Rough estimate
...
def get_bolt_conversation_id(request: ChatCompletionRequest) -> str:
    # Try to get a consistent ID from user messages, excluding system prompts or generic instructions
    ...
    return f"bolt-fallback-{uuid.uuid4()}" # Fallback if no suitable user message
...
async def chat_completions(request: ChatCompletionRequest):
    try:
        ...
        if bolt_conv_id in conversation_map:
            ...
            grok_conversation_id_to_map = grok_conv_id_from_map # Keep the existing mapping
        else:
            ...
            # The actual Grok ID will be captured from the generator stream for new conversations

        if grok_response_generator is None: # Should not happen with current logic
            raise HTTPException(status_code=500, detail="Failed to initialize Grok conversation generator")

        # Wrapper to capture the Grok conversation ID from the stream for mapping
        async def stream_wrapper_for_mapping(gen: AsyncGenerator[Dict, None], current_bolt_id: str) -> AsyncGenerator[Dict, None]:
            nonlocal grok_conversation_id_to_map # Allow modification of outer scope variable
            ...
            async for item in gen:
                if not id_captured_for_this_stream and item["type"] == "conversation_id":
                    ...
                    grok_conversation_id_to_map = new_grok_id # Ensure it's set for non-streaming collection
        ...
        if request.stream:
            ...
            # Ensure mapping is saved even if only non-streaming path is hit and ID was found
...
# --- Main execution block (auto-run server) ---
if __name__ == "__main__":
    import uvicorn
    ...
    # Check if .env was loaded by checking a key cookie
    if not os.getenv("GROK_CF_CLEARANCE"):
    ...
    except Exception as e: # Catch common errors like port in use
