    ...
    try:
        ...
        async for item in processed_gen:
            ...
            if item["type"] == "error" and "Conversation" in item["error"] and "not found" in item["error"] and not retry_attempted:
                ...
                if current_time - LAST_NEW_CONVERSATION_TIME < NEW_CONVERSATION_COOLDOWN:
                    ...
                    yield {"type": "error", "error": "Conversation reuse failed, and new conversation creation is throttled."}
                    needs_retry = False
                    break
                else:
        ...
        if request.stream:
            return StreamingResponse(
                stream_response(processed_gen, request, bolt_conversation_id),
            ...
            response = await collect_full_response(processed_gen)
