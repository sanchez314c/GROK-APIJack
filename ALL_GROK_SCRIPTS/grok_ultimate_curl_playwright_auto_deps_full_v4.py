...
async def collect_full_response(response_gen):
    ...
    try:
        ...
        if not final_message:
            final_message = "Fallback response: Grok did not return content. Please try again."
            logger.warning("No content received from Grok, using fallback message.")
...
async def chat_completions(request: ChatCompletionRequest, client: Request):
    ...
    try:
        ...
        # Always return non-streaming response for compatibility
        ...
        logger.info(f"Returning JSON response for {bolt_conversation_id} with ID {response_payload['id']}")
        logger.debug(f"Response payload being sent: {json.dumps(response_payload)[:500]}...")
        return JSONResponse(
